...
interact with each other. By default each components are deployed in different VMs
but there is an option to deploy all components in a single VM, see these sections of
[terraform](/docs/provision.md#deploy-all-bap-services-in-a-single-vm-opcional) and [ansible](/docs/deployment_and_config.md#deploy-all-bap-services-in-a-single-vm-optional). These components are:
